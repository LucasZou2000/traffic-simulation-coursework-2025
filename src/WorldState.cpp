...
void WorldState::CreateRandomWorld(int world_width, int world_height) {
	...
	// Clear any existing generated instances
	...
	for (const auto& pair : db_manager.building_database) {
		if (pair.first == 256) continue; // already placed
		const Building& template_building = pair.second;
		...
		while (!placed && attempts < building_max_attempts) {
			...
			if (valid_position) {
				Building new_building = template_building;
				new_building.x = x;
				new_building.y = y;
				buildings[new_building.building_id] = new_building;
				placed = true;
			}
			attempts++;
		}

...
std::map<int, int> Scheduler::computeShortage(const TaskGraph& g, const std::map<int, Item>& inventory) const {
	std::map<int, int> need;
	for (const TFNode& n : g.all()) {
...
std::vector<std::pair<int, int> > Scheduler::assign(const TaskGraph& g, const std::vector<int>& ready,
                                                    const std::vector<Agent*>& agents,
                                                    const std::map<int, int>& shortage) {
	std::vector<std::pair<int, int> > result;
	bundles_.assign(agents.size(), std::vector<int>());
	winners_.assign(g.all().size(), WinInfo());
	...
	for (int round = 0; round < MAX_ROUND; ++round) {
		...
		for (size_t ai = 0; ai < agents.size(); ++ai) {
			...
			for (size_t j = 0; j < ready.size(); ++j) {
				int tid = ready[j];
				double score = scoreTask(g.get(tid), *agents[ai], shortage);
...
void Scheduler::stepExecute(const std::vector<std::pair<int, int> >& plan, TaskGraph& g,
